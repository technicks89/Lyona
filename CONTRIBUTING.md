...
Thanks for helping improve lyona. Changes should preserve the small X11
window-manager core, existing user workflows, and the Arch Linux-only desktop
target. Arch Linux is the sole supported distribution.
...
On the supported Arch release, install the repository's Arch build
...
| Installer or package mapping | `scripts/run-tests make check-arch-packages` on Arch |
| Arch archiso | `scripts/run-tests make check-archiso` plus all evidence required by [SPEC.md Section 9.4](SPEC.md#94-arch-image-validation) |
| Release automation | `scripts/run-tests make release-check` and a dry run of the release helper |

Arch package, VM, and X11 checks require their documented host tools. If a
...
- Preserve the C99 style and avoid new mandatory dependencies unless they are
  available on supported Arch releases.
...
validation, Arch coverage, and remaining risk. Screenshots are useful for
