...
Upstream: `#211`-`#225` (fifteen commits). Doc: `docs/SYNC-P5-OPERATION-JOURNAL.md`.
Decision recorded at the close of Sync Phase 4: build the journal, since
`pacman.log` cannot answer "did *my* confirmed operation dispatch," "was the
result acknowledged," or "was a `timezone-set` confirmed" — and Sync Phase 9's
regional mutations have no package-manager log to fall back on at all. Ships
no user-visible behaviour: it is the crash-durable record
[Sync Phase 6](docs/SYNC-P6-UPDATE-EXECUTION.md) writes into.

Ported as nine sub-boundaries (J1-J9), one Lyona commit per boundary
(`sync-p5-operation-journal` branch), matching real upstream commit
dependency order rather than file-number order where the two diverged
(`#223`'s `commit_writable_journal_path` calls a helper `#221` introduces,
so J8 landed before J7 finished):

- [x] J1 (`ab8ca856`, #211) — the fixed 8192-byte double-buffered frame codec:
  `encode_journal_frame`/`decode_journal_frame`, every canonical boundary
  check (magic, version, reserved bytes, payload length, digest, padding),
  `select_journal_frame`'s highest-valid-sequence selection. — **Met**, 36
  tests.
- [x] J2 (`10ea44d6`, #212) — descriptor validation (regular file, 0600,
  single link, owned, correct access mode), the `flock`-based directory
  lock (5s deadline, TOCTOU identity re-check), and
  initialize/read/commit over the dual-frame file via
  `pread`/`pwrite`/`fsync` with readback verification;
  `JournalCommitError` on any indeterminate commit. — **Met**, 44 tests.
- [x] J3 (`e07d546f` + `e42f4717`, #213-#214) — the fixed 36-name layout
  (`active`/`restart`/`handoff`/32 terminal slots/`cursor`, cursor
  committed last) over a root-anchored `JournalDirectoryChain` that opens
  every path component as a held descriptor, `openat`-relative throughout.
  **Lyona rename applied**: `JOURNAL_DIRECTORY_SUFFIX = ("lyona",
  "system-management")`, not upstream's `dwm-titus` — the helper itself
  keeps its `dwm-` name per the standing table. — **Met**, 67 tests.
- [x] J4 (`7564b01f`, #215) — partial-layout recovery: every fixed path
  opened partially first, a bounded empty-file mode repair only when the
  cursor is absent, safe-fragment detection distinguishes a torn write
  from foreign content, a file already valid after the cursor is
  validated and never rewritten. — **Met**, 81 tests.
- [x] J5 (`4a4c632b` + `292d7de0`, #216-#217) — the control record codecs
  (two-digit cursor, operation-to-slot handoff, restart guidance) and the
  16-field `JournalOperation` record. `interrupted` is a first-class
  terminal state distinct from `failed`/`canceled`, per the doc's
  explicit instruction — Sync Phase 9 depends on that distinction.
  `decode_journal_operation` re-encodes and rejects any mismatch. —
  **Met**, 94 tests.
- [x] J6 (`2ae83a46` + `053b1969`, #218-#219) — `decode_journal_state`
  cross-validates the whole fixed record set (cursor freshness, handoff
  identity, old-boot clearing, restart-guidance strength/cutoff
  derivation with the ring-wrap escape hatch), and `load_journal_state`
  opens all 36 paths read-only through bound descriptors, never
  enumerating the directory. `_close_descriptors` shared with
  `JournalDirectoryChain.close()`. — **Met**, 115 tests.
- [x] J7 (`555c0c62` + `b6cee00b`, #220 and #223) —
  `open_writable_journal`/`JournalDescriptorSet` retain the complete
  writable journal under one exclusive lock, addressed by name; identity
  re-validated on entry and exit. `commit_writable_journal_path` commits
  one retained path between complete identity checks and never masks an
  indeterminate `JournalCommitError` behind a secondary validation
  failure. Landed as two commits (J7a before J8, J7b after) because
  `#223` depends on a helper `#221` (J8) introduces. — **Met**, 139 tests
  combined with J8.
- [x] J8 (`5b43dbbc` + `97bf6eff`, #221-#222) — `prepare_journal_admission`
  refuses a new operation while one is active, a handoff is
  unacknowledged, or any control path lacks its own admission's commit
  headroom (`JOURNAL_ACTIVE_ADMISSION_COMMITS = 12`, derivation comment
  ported verbatim); neither commits nor retains a lease.
  `generate_journal_operation_id` draws from the kernel CSPRNG, retrying
  up to `JOURNAL_OPERATION_ID_ATTEMPTS = 4` times against every retained
  ID. — **Met**, 134 tests.
- [x] J9 (`6662576c` + `c8585a80`, #224-#225) — full lifecycle:
  `begin_journal_operation` (durable pending before any mutation),
  `advance_journal_operation` (legal-transition and monotonic-strength
  checked, rejects progress-only writes), `prune_journal_restart` and
  `complete_journal_terminal` (idempotent terminalization through
  handoff, boot/login-boundary aware), `retained_journal_operation` /
  `acknowledge_journal_handoff`. `#224` also corrects
  `JOURNAL_PACKAGEKIT_PATH_PATTERN` to PackageKit's real root-level
  transaction ID shape (`/18_adcbcaed`, not a namespaced child path) —
  Option A keeps this pattern unchanged on Arch, per the doc. `#225`
  splits `open_writable_journal` into `retain_writable_journal` (holds
  file identity, not the lock, across external service waits) and
  `lock_writable_journal` (bounded exclusive reacquisition per
  checkpoint, rejecting a stale owner or unlocked access via
  `_require_exclusive_journal`); `open_writable_journal` itself becomes a
  thin wrapper over the same retained-files primitive. Also ported the
  17-line `SettingsModel.qml` refresh-sequencing change (`inputRefreshPending`,
  mirroring the existing `displayRefreshPending` pattern) named in the
  phase doc's file table. — **Met**, 161 tests.

Acceptance:

- `scripts/run-tests /usr/bin/python3 tests/test-system-management.py`,
  `scripts/run-tests make check-system-management` — **Met**, 161 tests,
  0 failures.
- `make check-quickshell-qml` — **Met**, no new warnings from the
  `SettingsModel.qml` change.
- `make check-settings` — **Met**.
- Manual, on a real CachyOS install (corrupt-frame recovery, group-writable
  directory refusal, relative `XDG_STATE_HOME` fallback, the full
  `test-quickshell-settings-xvfb.sh` race fixture upstream added alongside
  `#224` for holding a stale input-discovery result across a preview
  Keep) — **not run in this sandbox**; no real filesystem-permission
  attacker model or Xvfb session available here. The xvfb fixture itself
  was not ported — it exercises Settings-pane UI timing orthogonal to the
  journal, already covered structurally by the existing
  `displayRefreshPending` precedent this change mirrors exactly.
