...
- Displays, monitor profiles, and Xft display scaling. Placement uses
  numbered monitor cards and a proportional preview, with Left of, Right of,
  Above, and Below actions relative to another enabled monitor. Placement
  accounts for resolution and rotation, aligns the top or left edges, and
  retains the timed preview/rollback and coordinate-based profile formats.
  Choosing a placement edits the pending layout only.
...
A launch that Picom refuses because another compositor selection owner exists is
retried, up to three times, only when that owner releases the selection within
two seconds. The status probe (`picom --diagnostics`) claims the selection
briefly, so a Settings refresh can otherwise make a valid start fail; a real
second compositor keeps the selection and still fails immediately.
