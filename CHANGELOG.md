...
- A cross-tag window overview popup, mouse-only (Sync Sprint 7 S7-03,
  `docs/SYNC-SPRINT-7-OVERVIEW-FOUNDATION.md`, part of the cross-tag window overview, issue `#350`), bound to `SUPER+o`
  (pick your own binding if you'd rather have it elsewhere; `SUPER+Tab` was the design doc's own first candidate but is
  already dwm's "Previous tag"). `config/quickshell/overview/WindowOverview.qml`, a `ClickAwayPopup` like `ControlsWindow`,
  lists every open window across every tag, one `SectionLabel` group per occupied tag (the launcher's own "Categories"
  component, reused), with an icon/title/monitor card per window (`OverviewCard.qml`); clicking a card calls
  `DwmState.focusWindow(windowId)` directly, the same function `RunningAppsArea.qml`'s own panel click handler already
  calls, then closes the popup. `OverviewModel.qml` is thin glue (visibility, which screen it opened on) over a new
  `DwmStateWindows.js` function, `groupByTag()`, unit-tested directly via `tests/qml/tst_dwm_state_windows.qml` the same
  way S7-02's own `windowsByTag()` is. `shell.qml` gains an `IpcHandler { target: "overview" }` (`open()`/`close()`/
  `toggle()`/`windowCount()`) and closes the overview from every other popup's own open path (`selectPanelPopup`,
  `openCommandMenu`, the launcher), and closes every other popup from its own `openOverview()`, matching the existing
  cross-popup closing convention rather than inventing a parallel one. Keyboard navigation between cards, multi-monitor
  label polish and the window-closes-while-open edge case are Sprint 8, not this item; live thumbnails, motion and an
  accessibility pass are Sprint 9.
