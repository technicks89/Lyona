...
struct Monitor {
	...
	unsigned int tagmask;   /* cached getmontagmask(num) */
	int tagmaskvalid;
	int showbar;
	int topbar;
	Client *clients;
...
static void focusfirsttagged(unsigned int tags);
...
/* Set whenever a top-level window appears or the tray goes away. The three
 * `if (!m->traywin) scantray()` sites gate on it so a missing tray no longer
 * costs a full root XQueryTree plus a WM_CLASS round-trip per top-level
 * window on every Expose -- the tree cannot have grown a tray since the last
 * scan unless something mapped. */
static int trayscanpending = 1;
static int monitorcount = 1;
static int getmonlogicalindex(Monitor *target);
static unsigned int computemontagmask(int monnum);
static unsigned int getmontagmask(int monnum);
static void invalidatemontagmasks(void);
static Monitor *montagowner(unsigned int tags);
...
#if 0 /* disabled: internal bar drawing, superseded by the external
         Quickshell panel. Unreachable since an early return was added
         to drawbar(); kept for reference until the panel has soaked. */
void
drawbar(Monitor *m)
{
	int x, w, tw = 0;
...
scan(void)
{
	...
	/* Transients have to be managed after their parents, hence two passes.
	 * Probing each window once up front rather than once per pass halves the
	 * attribute round-trips and drops a redundant WM_TRANSIENT_FOR fetch for
	 * every window on the display. */
	struct {
		XWindowAttributes wa;
		int probed;
		int transient;
	...
	probe = ecalloc(num ? num : 1, sizeof *probe);
	for (i = 0; i < num; i++) {
		if (!XGetWindowAttributes(dpy, wins[i], &probe[i].wa))
			continue;
		probe[i].probed = 1;
		probe[i].transient = XGetTransientForHint(dpy, wins[i], &d1) != 0;
	}

	for (i = 0; i < num; i++) {
		if (!probe[i].probed)
			continue;
		if (probe[i].wa.override_redirect) {
			if (probe[i].wa.map_state == IsViewable)
				trackoverridewindow(wins[i]);
			continue;
		}
		if (probe[i].transient)
			continue;
...
setup(void)
{
	...
	// cursor[CurResize] = drw_cur_create(drw, XC_sizing); /* disabled: slot never read */
...

	{
		static const char *init_clrs[2][3] = {
			{ "#D8DEE9", "#434C5E", "#3B4252" },
			{ "#ECEFF4", "#434C5E", "#81A1C1" },
		};
...
/* Launch through dwm-session-launch so the spawned application picks up the
 * current cursor and Qt settings. dwm's own environment was fixed at login,
 * so without this every keybind hands out whatever the theme was then.
 *
 * The wrapper is resolved beside the running binary rather than through PATH,
 * so a custom install prefix cannot go stale. Running ./dwm straight from the
 * build tree has no sibling wrapper: that takes the fallback and simply loses
 * the refresh. */
void
spawn(const Arg *arg)
{
...
togglebar(const Arg *arg)
{
	...
	if (!selmon->traywin && trayscanpending)
		scantray();
...
#if 0 /* disabled: dwm no longer creates its own bar window; the external
         Quickshell panel is adopted through updatealtbar() instead. */
void
updatebars(void)
{
	Monitor *m;
...
/* Cache WM_CLASS on the client. attachbottom() inspects every existing client
 * on the monitor and used to issue one XGetClassHint round-trip per client per
 * attach, which reconcilemonitortags() turned into a monitors x clients^2 walk
 * on every hotplug. WM_CLASS is fixed for the lifetime of nearly every window,
 * and propertynotify() refreshes the cache for the ones that set it late.
 * Both fields are left empty when the window has no WM_CLASS. */
void
updateclass(Client *c)
{
	...
	if (!XGetClassHint(dpy, c->win, &ch))
		return;
	if (ch.res_class) {
		copystr(c->class, sizeof c->class, ch.res_class);
		XFree(ch.res_class);
	}
	if (ch.res_name) {
		copystr(c->instance, sizeof c->instance, ch.res_name);
		XFree(ch.res_name);
	}
}
...
updategeom(void)
{
	...
	if (dirty) {
		/* Monitor numbers and geometry both feed the cached tag masks. */
		invalidatemontagmasks();
		selmon = mons;
...
/* Focus the first visible client on the selected monitor carrying any of
 * `tags` and warp the pointer onto it. With no such client the pointer goes to
 * the monitor centre instead, so a warp-enabled view() never leaves it behind
 * on the monitor it came from. */
static void
focusfirsttagged(unsigned int tags)
{
	Client *c;

	for (c = selmon->clients; c; c = c->next) {
		if ((c->tags & tags) && ISVISIBLE(c)) {
			focus(c);
			if (cursorwarp)
...
/* First monitor whose tag mask covers any bit in `tags`, or NULL when no
 * monitor claims them. */
static Monitor *
montagowner(unsigned int tags)
{
	Monitor *m;

	for (m = mons; m; m = m->next)
		if (tags & getmontagmask(m->num))
			return m;
	return NULL;
}

/* getmontagmask() walks `mons` and then getmonlogicalindex() walks it twice
 * more, and the result depends only on the monitor layout -- so it is cached
 * per monitor and recomputed only here. */
static unsigned int
computemontagmask(int monnum)
{
...
getmontagmask(int monnum)
{
	...
	for (m = mons; m; m = m->next) {
		if (m->num == monnum) {
			if (!m->tagmaskvalid) {
				m->tagmask = computemontagmask(monnum);
				m->tagmaskvalid = 1;
			}
			return m->tagmask;
		}
	}
	/* No monitor owns this number, so there is nowhere to cache it. */
	return computemontagmask(monnum);
}

/* The cached masks depend on the monitor list and on monitorcount, both of
 * which only change in updategeom(). */
void
invalidatemontagmasks(void)
{
	Monitor *m;
