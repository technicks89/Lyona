...
if $APPLY || $APPLY_TLP; then
	...
	if [[ $DISTRO_ID != arch ]]; then
		echo "ERROR: --apply and --apply-tlp support Arch Linux only." >&2
		exit 1
	fi
	if [[ $EUID -ne 0 ]]; then
...
package_install_hint() {
	...
	if command -v pacman &>/dev/null; then
		...
		printf 'install Arch package: %s' "$package"
	fi
}
...
if [[ -n "$BAT_PATH" ]]; then
	...
	warn "No battery found — may be running on AC only or battery sensor missing"
fi
...
if command -v tlp-stat &>/dev/null; then
	...
	get_tlp() {
		awk -F= "!/^[[:space:]]*#/ && /^${1}=/{print \$2}" "$TLP_CONF" 2>/dev/null | tail -1
	}
...
_tlp_conf_ok() {
	local key="$1" want="$2"
	local val
	val=$(awk -F= "!/^[[:space:]]*#/ && /^${key}=/{print \$2}" "$TLP_CONF" 2>/dev/null | tail -1)
	[[ "$val" == "$want" ]]
}
TLP_INSTALLED=false
