...
HELPER="$repo/scripts/dwm-terminal"
BASH_BIN="${BASH:-/usr/bin/bash}"

work="$(mktemp -d)"
trap 'rm -rf "$work"' EXIT

mkdir -p "$work/bin" "$work/home/.config/lyona"
export HOME="$work/home"
export XDG_CONFIG_HOME="$HOME/.config"

cat >"$XDG_CONFIG_HOME/lyona/hotkeys.toml" <<'EOF'
[vars]
terminal = "alacritty"
EOF
...
sed -i 's/terminal = "alacritty"/terminal = "kitty"/' \
	"$XDG_CONFIG_HOME/lyona/hotkeys.toml"
...
grep -Eq '^[[:space:]]*terminal[[:space:]]*=[[:space:]]*"alacritty"' \
	"$repo/config/hotkeys.toml"
grep -Fq 'key="x",       desc="Terminal"' "$repo/config/hotkeys.toml"
