...
cp "$repo/config/quickshell/assets/lyona-icon.png" "$home/Pictures/backgrounds/test-wallpaper.png"
...
Exec=/usr/bin/true
EOF
printf 'Xft.dpi: 144\n' >"$config_home/lyona/dpi.Xresources"
...
grep -Fqx "$sticky_record" "$config_home/lyona/input-settings.conf"
i=0
while [ "$i" -lt 100 ]; do
	preview_state=$(DISPLAY=$display HOME=$home XDG_CONFIG_HOME=$config_home XDG_DATA_HOME=$data_home \
		XDG_RUNTIME_DIR=$runtime quickshell ipc --path "$config" call settings inputPreviewState)
...
[ "$sticky_value" = "$sticky_baseline" ]
[ "$sticky_live" = "$sticky_baseline" ]
if [ -f "$config_home/lyona/input-settings.conf" ] &&
	grep -Fq "$(printf 'accessx\tsticky-keys\t')" "$config_home/lyona/input-settings.conf"; then
...
sed -i 's/terminal = "kitty"/terminal = "alacritty"/' "$config_home/lyona/hotkeys.toml"
i=0
while [ "$i" -lt 200 ]; do
	terminal_id=$(DISPLAY=$display HOME=$home XDG_CONFIG_HOME=$config_home XDG_DATA_HOME=$data_home \
		XDG_RUNTIME_DIR=$runtime quickshell ipc --path "$config" call settings defaultsRoleDesktopId terminal 2>/dev/null || true)
...
cp "$repo/config/quickshell/assets/lyona-icon.png" "$test_wallpaper"

transaction_state_file=$state_home/lyona/appearance/integration-transaction
...
cp "$config_home/lyona/themes.toml" "$work/valid-themes.toml"
