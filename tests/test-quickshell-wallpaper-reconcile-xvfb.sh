...
# A wallpaper preview that failed is reconciled by a queued retry. Whatever was
# blocking it (busy, a font change, another wallpaper action), the retry must
# run once that is over, and only once. Driven through the real shell model with
# every helper command stubbed out.
...
if [ "${DWM_WALLPAPER_RECONCILE_DBUS_SESSION:-0}" != 1 ]; then
	exec env DWM_WALLPAPER_RECONCILE_DBUS_SESSION=1 dbus-run-session -- "$0" "$@"
fi
...
python3 "$repo/tests/fixtures/wallpaper-reconcile.py" "$work/qml" "$repo/tests/qml/WallpaperReconcile.inc"
...
if [ "$status" -ne 0 ] || ! grep -F 'Wallpaper reconcile: PASS' "$work/ui.log" ||
	grep -Eq 'Wallpaper reconcile FAILED:|ReferenceError:|TypeError:' "$work/ui.log"; then
	cat "$work/ui.log" >&2
	exit 1
fi
printf 'Wallpaper reconcile: PASS\n'
