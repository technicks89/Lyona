...
# The watcher itself lives in core/WatchedProcess.qml now; the model has to
# reach it, with the guard that stops the restart timer when hidden.
...
assert_contains "$autostart_model" 'WatchedProcess {'
assert_contains "$autostart_model" 'active: root.settingsVisible'
