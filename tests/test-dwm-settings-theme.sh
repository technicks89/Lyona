...
themes_fixture=$work/themes-fixture.toml
sed '0,/^theme = "[^"]*"/s//theme = "nord"/' "$repo/config/themes.toml" \
	>"$themes_fixture"
...
reset_fixture() {
	...
	mkdir -p "$config_home/lyona" "$state_home" "$runtime_dir"
	cp "$themes_fixture" "$themes_file"
...
mkdir -p "$relative_home/.config/lyona" \
	"$relative_home/.local/share/lyona/config" "$relative_runtime"
cp "$themes_fixture" "$relative_home/.config/lyona/themes.toml"
cp "$themes_fixture" "$relative_home/.local/share/lyona/config/themes.toml"
(
	cd "$work"
...
grep -Fq 'theme = "dracula"' "$relative_home/.config/lyona/themes.toml"
...
[[ $(active_theme) == nord ]]
[[ ! -e $state_home/lyona/appearance/transaction.meta ]]
...
exchange_name=$(awk -F= '$1 == "exchange_file" { print $2; exit }' \
	"$state_home/lyona/appearance/transaction.meta")
...
slow_prefix=$state_home/lyona/appearance/slow-preview.preview
...
cancel_prefix=$state_home/lyona/appearance/cancelled-watchdog.preview
...
mkdir -p "$state_home/lyona/appearance/orphaned-claim.preview.claim"
wait_for_theme nord
grep -Fqx $'result\tnone' < <(run_theme preview-status)

# A toolkit preview blocks theme mutations, mirroring the gate
# dwm-settings-toolkit already applies in the other direction.
#
# The toolkit's preview leaves personalization.conf provisional with a rollback
# armed against a baseline of the files theme-apply.sh derives. Switching the
# theme underneath rewrites those same files, so that rollback would restore
# against a system it no longer describes.
reset_fixture
toolkit_preview=$state_home/lyona/appearance/toolkit/preview.current
mkdir -p "${toolkit_preview%/*}"
printf 'tok-cross\n' >"$toolkit_preview"

if run_theme apply nord >/dev/null 2>"$work/toolkit-gate-apply.err"; then
	printf 'a theme apply was allowed while a toolkit preview was armed\n' >&2
	exit 1
fi
grep -Fq 'a toolkit preview is active' "$work/toolkit-gate-apply.err"
if run_theme preview tok-theme 10 nord >/dev/null 2>"$work/toolkit-gate-preview.err"; then
	printf 'a theme preview was allowed while a toolkit preview was armed\n' >&2
	exit 1
fi
grep -Fq 'a toolkit preview is active' "$work/toolkit-gate-preview.err"

# Settings asks before it offers, so the pane can be disabled rather than
# letting the user find out by being refused.
if run_theme mutation-ready >/dev/null 2>&1; then
	printf 'mutation-ready did not report the toolkit preview\n' >&2
	exit 1
fi

# A stale failed preview holds the gate shut too: it is unresolved either way.
rm -f -- "$toolkit_preview"
printf 'tok-cross\tsomething went wrong\n' >"${toolkit_preview%/*}/preview.failed"
if run_theme apply nord >/dev/null 2>&1; then
	printf 'a theme apply was allowed while a failed toolkit preview was unresolved\n' >&2
	exit 1
fi
if run_theme mutation-ready >/dev/null 2>&1; then
	printf 'mutation-ready did not report the failed toolkit preview\n' >&2
	exit 1
fi

# Once the toolkit preview is resolved the theme helper works again.
rm -f -- "${toolkit_preview%/*}/preview.failed"
run_theme mutation-ready >/dev/null 2>&1
run_theme apply nord >/dev/null 2>"$work/toolkit-gate-clear.err"
...
chmod 640 "$config_home/alacritty/alacritty.toml" "$config_home/kitty/kitty.conf" \
	...
	"$config_home/lyona/cursor.Xresources" "$config_home/lyona/theme-env.sh" \
	"$config_home/qt6ct/qt6ct.conf"
...
integration_transaction=$state_home/lyona/appearance/integration-transaction
...
suppression_file=$state_home/lyona/appearance/integration-suppress
...
grep -Fqx '# external integration edit before publish' \
	"$config_home/alacritty/alacritty.toml"
[[ -f $state_home/lyona/appearance/integration-publish-race.preview.meta ]]
...
durable_prefix=$state_home/lyona/appearance/durable-preview.preview
...
printf 'orphan-preview\n' >"$state_home/lyona/appearance/preview.current"
grep -Fqx $'result\tnone' < <(run_theme preview-status)
[[ ! -e $state_home/lyona/appearance/preview.current ]]
...
[[ ! -d $state_home/lyona/appearance/stale-keep.preview.claim ]]
...
[[ ! -e $state_home/lyona/appearance/integration-transaction ]]
...
cmp -s <(printf '%s\n' "$suppression_hash") \
	"$state_home/lyona/appearance/integration-suppress"
...
fallback_output=$(HOME=$fallback_home XDG_CONFIG_HOME=$fallback_config XDG_DATA_HOME=$data_home \
	...
	DWM_TEST_LIVE_LOG=$work/live.log PATH=$work/integration-bin:$PATH \
	DWM_APPEARANCE_THEMES_FILE=$fallback_config/lyona/themes.toml \
	DWM_APPEARANCE_MANAGED_THEMES_FILE=$managed_file \
	"$repo/scripts/theme-apply.sh" 2>"$work/fallback.err")
grep -Fqx "theme-apply: applied theme 'nord'" <<<"$fallback_output"
# Either the palette's generated theme, or the built-in fallback when the
# generated one is not installed in this fixture.
grep -Eq '^gtk-theme-name="(Lyona-[a-z-]+|Adwaita-dark)"$' "$fallback_home/.gtkrc-2.0"
...
# The sealed PATH pins theme-apply.sh's dependency surface. mktemp and mv are
# in it because the GTK ini writers now stage into a temporary file and rename
# it over the target, rather than editing in place with sed -i.
