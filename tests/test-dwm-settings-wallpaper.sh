...
if [[ $loadable_mode == true ]]; then
	if [[ -n ${DWM_TEST_FEH_LOADABLE_ARGS_FILE:-} ]]; then
		for argument in "$@"; do
			printf 'loadable-arg=%s\n' "$argument" >>"$DWM_TEST_FEH_LOADABLE_ARGS_FILE"
		done
	fi
	...
	for argument in "$@"; do
		...
		# A directory as the fail path means nothing under it loads. The
		# helper now names individual files here rather than handing feh the
		# wallpaper folder, so the containment check is what carries that.
		[[ -z ${DWM_TEST_FEH_FAIL_PATH:-} || ! -d $DWM_TEST_FEH_FAIL_PATH ||
			$argument != "$DWM_TEST_FEH_FAIL_PATH"/* ]] || continue
		if [[ -f $argument ]]; then
			printf '%s\n' "$argument"
...
grep -Fqx $'mutation\trestricted\tWallpaper changes are unavailable in this session' \
	<<<"$read_only_initial"
test ! -e "$config_home/lyona"
test ! -e "$state_home/lyona"
...
test ! -e "$state_home/lyona/appearance/wallpaper/mutation.failed"
...
multi_monitor_meta=$state_home/lyona/appearance/wallpaper/multi-monitor-baseline.meta
...
fit=max
EOF
cmp "$work/expected.conf" "$config_home/lyona/wallpaper.conf"
test "$(stat -c %a "$config_home/lyona/wallpaper.conf")" = 600
test "$(stat -c %a "$config_home/lyona")" = 750
...
# every image in it on each login with no wallpaper configured.
loadable_args_file=$work/loadable-args
: >"$loadable_args_file"
saved_default_config=$work/saved-default.conf
cp -- "$config_home/lyona/wallpaper.conf" "$saved_default_config"
rm -f -- "$config_home/lyona/wallpaper.conf"
export DWM_TEST_FEH_LOADABLE_ARGS_FILE=$loadable_args_file
run_helper session-apply >/dev/null
unset DWM_TEST_FEH_LOADABLE_ARGS_FILE
if grep -Fqx "loadable-arg=$wallpaper_dir" "$loadable_args_file"; then
	printf 'Default wallpaper selection probed the whole wallpaper folder\n' >&2
	exit 1
fi
if grep -Fqx 'loadable-arg=--recursive' "$loadable_args_file"; then
	printf 'Default wallpaper selection still probes recursively\n' >&2
	exit 1
fi
grep -q "^loadable-arg=$wallpaper_dir/" "$loadable_args_file"
probed=$(grep -c "^loadable-arg=$wallpaper_dir/" "$loadable_args_file")
# One monitor plus the spare head-room, never the whole folder.
test "$probed" -le 5
mv -- "$saved_default_config" "$config_home/lyona/wallpaper.conf"
...
cp -- "$config_home/lyona/wallpaper.conf" "$work/before-randomize.conf"
...
cmp "$work/before-randomize.conf" "$config_home/lyona/wallpaper.conf"
grep -Fqx "path=$second" "$state_home/lyona/appearance/wallpaper/session.default"
...
grep -Fqx "arg=$first" "$log"
grep -Fqx 'arg=--bg-max' "$log"
test ! -e "$state_home/lyona/appearance/wallpaper/session.default"
...
grep -Fqx "xauthority=$relative_session/.Xauthority" \
	"$state_home/lyona/appearance/wallpaper/relative-authority.meta"
: >"$log"
run_helper revert relative-authority >/dev/null
grep -Fqx "xauthority=$relative_session/.Xauthority" "$log"
grep -Fqx "path=$first" "$config_home/lyona/wallpaper.conf"
grep -Fqx 'fit=max' "$config_home/lyona/wallpaper.conf"
...
test ! -s "$log"
preview_meta=$state_home/lyona/appearance/wallpaper/keep-me.meta
...
grep -Fqx 'fit=tile' "$config_home/lyona/wallpaper.conf"
...
grep -Fqx 'version=3' "$state_home/lyona/appearance/wallpaper/selection.owner"
...
grep -Fqx "baseline_path=$first" \
	"$state_home/lyona/appearance/wallpaper/degraded-baseline.meta"
...
rearm_meta=$state_home/lyona/appearance/wallpaper/rearm-me.meta
...
cat >"$config_home/lyona/wallpaper.conf" <<EOF
version=1
path=$second
fit=center
EOF
...
test ! -e "$state_home/lyona/appearance/wallpaper/preview.current"
grep -Fqx "path=$second" "$config_home/lyona/wallpaper.conf"
grep -Fqx 'fit=center' "$config_home/lyona/wallpaper.conf"
...
ln -s "$symlink_failure_target" \
	"$state_home/lyona/appearance/wallpaper/symlinked-failure.failed"
cat >"$config_home/lyona/wallpaper.conf" <<EOF
version=1
path=$first
fit=max
...
test ! -L "$state_home/lyona/appearance/wallpaper/symlinked-failure.failed"
test "$(stat -c %h "$state_home/lyona/appearance/wallpaper/symlinked-failure.failed")" = 1
...
ln "$hardlink_failure_target" \
	"$state_home/lyona/appearance/wallpaper/hardlinked-failure.failed"
cat >"$config_home/lyona/wallpaper.conf" <<EOF
version=1
path=$second
fit=center
...
test "$(stat -c %h "$state_home/lyona/appearance/wallpaper/hardlinked-failure.failed")" = 1
...
external_expiry_meta=$state_home/lyona/appearance/wallpaper/external-expiry.meta
...
printf '%s\n' 'Configured and fallback wallpapers are unavailable' \
	>"$state_home/lyona/appearance/wallpaper/failed-abandon.failed"
rm -f -- "$second"
...
printf 'second image\n' >"$second"
export DWM_TEST_FEH_MUTATE_CONFIG=$config_home/lyona/wallpaper.conf
...
grep -Fqx 'version=1' "$config_home/lyona/wallpaper.conf"
grep -Fqx 'mode=default' "$config_home/lyona/wallpaper.conf"
...
printf '%s\n' 'Wallpaper live rollback failed; retry session recovery before previewing' \
	>"$state_home/lyona/appearance/wallpaper/mutation.failed"
chmod 600 "$state_home/lyona/appearance/wallpaper/mutation.failed"
export DWM_TEST_FEH_FAIL_ALL=1
reset_output=$(run_helper reset)
grep -Fqx $'result\treset\tunavailable' <<<"$reset_output"
test -f "$state_home/lyona/appearance/wallpaper/mutation.failed"
...
export DWM_TEST_DEFAULT_CANDIDATE=$first
run_helper session-apply
grep -Fqx "path=$first" "$state_home/lyona/appearance/wallpaper/session.default"
...
rm -f -- "$config_home/lyona/wallpaper.conf"
ln -s "$work/unsafe.conf" "$config_home/lyona/wallpaper.conf"
printf '%s\n' 'Wallpaper configuration became unsafe during preview' \
	>"$state_home/lyona/appearance/wallpaper/unsafe-config-abandon.failed"
chmod 600 "$state_home/lyona/appearance/wallpaper/unsafe-config-abandon.failed"
if run_helper mutation-ready; then
...
test ! -e "$state_home/lyona/appearance/wallpaper/preview.current"
test -L "$config_home/lyona/wallpaper.conf"
...
printf 'not a wallpaper config\n' >"$config_home/lyona/wallpaper.conf"
...
grep -Fqx 'not a wallpaper config' "$config_home/lyona/wallpaper.conf"
test ! -s "$log"
rm -f -- "$config_home/lyona/wallpaper.conf"
...
preview_current=$state_home/lyona/appearance/wallpaper/preview.current
...
EOF
chmod +x "$bin_dir/mv"
export DWM_TEST_REAL_MV=$real_mv DWM_TEST_CONFIG_FILE=$config_home/lyona/wallpaper.conf
export DWM_TEST_SESSION_DEFAULT=$state_home/lyona/appearance/wallpaper/session.default
...
test ! -e "$config_home/lyona/wallpaper.conf"
...
unset DWM_TEST_MV_MODE
test -d "$config_home/lyona/wallpaper.conf"
test -z "$(find "$config_home/lyona/wallpaper.conf" -mindepth 1 -print -quit)"
grep -Fq 'could not be published' "$work/directory-publish.err"
rmdir "$config_home/lyona/wallpaper.conf"
...
grep -Fqx "path=$second" "$config_home/lyona/wallpaper.conf"
grep -Fqx 'fit=tile' "$config_home/lyona/wallpaper.conf"
recovery_configs=("$config_home"/lyona/wallpaper.conf.staged.*)
...
test ! -e "$state_home/lyona/appearance/wallpaper/session.default"
test -z "$(find "$state_home/lyona/appearance/wallpaper" -maxdepth 1 \
...
test ! -e "$preview_current"
test ! -e "$state_home/lyona/appearance/wallpaper/interrupted-publish.meta"
...
[[ -s $preview_publish_ready ]]
killed_before_publish_meta=$state_home/lyona/appearance/wallpaper/killed-before-publish.meta
...
test ! -e "$state_home/lyona/appearance/wallpaper/preview.owner"
test ! -e "$preview_current"
...
test -f "$state_home/lyona/appearance/wallpaper/mutation.failed"
test -f "$state_home/lyona/appearance/wallpaper/mutation.owner"
recovery_status=$(run_helper status)
grep -Fqx $'mutation\trestricted\tWallpaper live rollback failed; retry session recovery before previewing' \
	<<<"$recovery_status"
...
cat >"$state_home/lyona/appearance/wallpaper/mutation.owner" <<'EOF'
version=1
token=mutation
display=:915
xauthority=
EOF
chmod 600 "$state_home/lyona/appearance/wallpaper/mutation.owner"
...
if compgen -G "$config_home/lyona/wallpaper.conf.staged.*" >/dev/null; then
...
display_preview_meta=$state_home/lyona/appearance/wallpaper/display-owned.meta
...
foreign_rearm_meta=$state_home/lyona/appearance/wallpaper/foreign-rearm.meta
...
reused_meta=$state_home/lyona/appearance/wallpaper/reused-watchdog-pid.meta
...
stale_display_meta=$state_home/lyona/appearance/wallpaper/stale-display.meta
...
test ! -e "$state_home/lyona/appearance/wallpaper/preview.current"
test ! -e "$state_home/lyona/appearance/wallpaper/transient-session-recovery.meta"
test ! -e "$state_home/lyona/appearance/wallpaper/transient-session-recovery.failed"
...
backward_clock_meta=$state_home/lyona/appearance/wallpaper/backward-clock.meta
...
fractional_deadline_meta=$state_home/lyona/appearance/wallpaper/fractional-deadline.meta
...
resume_deadline_meta=$state_home/lyona/appearance/wallpaper/resume-deadline.meta
grep -Fqx 'deadline=1120000' "$resume_deadline_meta"
: >"$log"
printf '1130.00 0.00\n' >"$fake_monotonic"
for _ in {1..60}; do
	[[ ! -e $state_home/lyona/appearance/wallpaper/preview.current ]] && break
	sleep 0.05
done
test ! -e "$state_home/lyona/appearance/wallpaper/preview.current"
...
stale_boot_meta=$state_home/lyona/appearance/wallpaper/stale-boot.meta
...
failed_display_meta=$state_home/lyona/appearance/wallpaper/failed-display.meta
...
printf '%s\n' 'Owner display rollback failed' \
	>"$state_home/lyona/appearance/wallpaper/failed-display.failed"
: >"$log"
run_helper_display :916 session-apply
test ! -e "$state_home/lyona/appearance/wallpaper/preview.current"
grep -Fqx 'display=:915' "$log"
...
reused_display_meta=$state_home/lyona/appearance/wallpaper/reused-display-failure.meta
...
printf '%s\n' 'Previous session rollback failed' \
	>"$state_home/lyona/appearance/wallpaper/reused-display-failure.failed"
run_helper_display :915 session-apply
test ! -e "$state_home/lyona/appearance/wallpaper/preview.current"
...
test ! -e "$find_log"
# Scoped to the status path: choose_default_wallpapers enumerates candidate
# names with find, so leaving this stub in place would break every later
# default selection.
rm -f -- "$bin_dir/find"
...
test ! -e "$state_home/lyona/appearance/wallpaper/preview.current"
test ! -e "$state_home/lyona/appearance/wallpaper/bounded-feh.failed"
...
test ! -e "$state_home/lyona/appearance/wallpaper/preview.current"
test ! -e "$state_home/lyona/appearance/wallpaper/metadata-write-failure.meta"
if grep -Fqx "arg=$second" "$log"; then
...
finish_clamp_meta=$state_home/lyona/appearance/wallpaper/finish-clamp-failure.meta
...
clamp_meta=$state_home/lyona/appearance/wallpaper/reconcile-clamp-failure.meta
...
rearm_failure_meta=$state_home/lyona/appearance/wallpaper/reconcile-rearm-failure.meta
...
mktemp_failure_meta=$state_home/lyona/appearance/wallpaper/rearm-mktemp-failure.meta
...
noncanonical_meta=$state_home/lyona/appearance/wallpaper/noncanonical-deadline.meta
...
corrupt_active_meta=$state_home/lyona/appearance/wallpaper/corrupt-active-meta.meta
...
corrupt_owner_meta=$state_home/lyona/appearance/wallpaper/corrupt-current-owner.meta
...
: >"$state_home/lyona/appearance/wallpaper/preview.current"
: >"$log"
...
mismatched_owner_meta=$state_home/lyona/appearance/wallpaper/mismatched-owner-token.meta
...
stale_owner_meta=$state_home/lyona/appearance/wallpaper/corrupt-current-stale-owner.meta
...
missing_current_meta=$state_home/lyona/appearance/wallpaper/missing-current-after-apply.meta
...
rm -f -- "$state_home/lyona/appearance/wallpaper/preview.current"
: >"$log"
...
grep -Fqx $'preview\tfailed\tmissing-current-after-apply\t0\t\tfill\tWallpaper preview state is invalid; restore it explicitly before changing wallpaper' \
	<<<"$owner_only_status"
test -f "$state_home/lyona/appearance/wallpaper/preview.owner"
...
overlapping_owner_meta=$state_home/lyona/appearance/wallpaper/overlapping-owner-recovery.meta
...
rm -f -- "$state_home/lyona/appearance/wallpaper/preview.current"
selection_owner=$state_home/lyona/appearance/wallpaper/selection.owner
selection_hash=$(sha256sum "$config_home/lyona/wallpaper.conf" | awk '{ print $1 }')
...
unset DWM_TEST_FEH_FAIL_DISPLAY
grep -Fqx 'display=:915' "$state_home/lyona/appearance/wallpaper/selection.owner"
...
unset DWM_TEST_FEH_FAIL_DISPLAY
test "$(grep -Fxc 'display=:915' "$state_home/lyona/appearance/wallpaper/selection.owner")" -eq 1
test "$(grep -Fxc 'display=:916' "$state_home/lyona/appearance/wallpaper/selection.owner")" -eq 1
test "$(stat -c %s "$state_home/lyona/appearance/wallpaper/selection.owner")" -gt 8192
unset XAUTHORITY
cat >"$bin_dir/xprop" <<'EOF'
#!/bin/sh
[ "${1:-}" = -display ] && [ "${2:-}" = :915 ]
...
test -f "$state_home/lyona/appearance/wallpaper/selection.owner"
test "$(grep -Fxc 'display=:915' "$state_home/lyona/appearance/wallpaper/selection.owner")" -eq 1
test "$(grep -Fxc 'display=:916' "$state_home/lyona/appearance/wallpaper/selection.owner")" -eq 0
run_helper_display :916 apply "$first" tile >/dev/null
test ! -e "$state_home/lyona/appearance/wallpaper/selection.failed"
test -f "$state_home/lyona/appearance/wallpaper/selection.owner"
...
selection_failure=$state_home/lyona/appearance/wallpaper/selection.failed
selection_owner=$state_home/lyona/appearance/wallpaper/selection.owner
sha256sum "$config_home/lyona/wallpaper.conf" | awk '{ print $1 }' >"$selection_failure"
cat >"$selection_owner" <<'EOF'
version=2
display=:915
xauthority=
...
cp -- "$config_home/lyona/wallpaper.conf" "$saved_selection_config"
rm -f -- "$config_home/lyona/wallpaper.conf"
...
test ! -e "$config_home/lyona/wallpaper.conf"
mv -- "$saved_selection_config" "$config_home/lyona/wallpaper.conf"

run_helper preview missing-active-meta 20 "$second" fill >/dev/null
missing_active_meta=$state_home/lyona/appearance/wallpaper/missing-active-meta.meta
...
cp -- "$config_home/lyona/wallpaper.conf" "$config_before_malformed_owner"
...
cmp -- "$config_before_malformed_owner" "$config_home/lyona/wallpaper.conf"
