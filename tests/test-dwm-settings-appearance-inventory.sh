...
proc_root=$work/proc
mkdir -p "$config_home/lyona" "$config_home/dconf" "$config_home/fontconfig" \
	...
	"$state_root/lyona/appearance/wallpaper" \
...
config_dir=$(readlink -m -- "$XDG_CONFIG_HOME/lyona")
appearance_state_dir=$(readlink -m -- "${XDG_STATE_HOME:-$HOME/.local/state}/lyona/appearance")
...
grep -Fqx $'selection\ticon\tunavailable\tPapirus\t\tConfigured selection is not installed' \
	<<<"$optional_loss_inventory"
grep -Fqx $'selection\tgtk\tunavailable\tLyona-nord\t\tConfigured selection is not installed' \
	<<<"$optional_loss_inventory"
...
grep -Fqx $'candidate\tgtk\tavailable\tLyona-nord\tLyona-nord\tGTK 3 and GTK 4 theme' <<<"$inventory"
...
grep -Fq -- "$state_root/lyona/appearance/wallpaper" "$watch_args"
...
grep -Fq -- "$data_root/themes/Lyona-nord" "$watch_args"
