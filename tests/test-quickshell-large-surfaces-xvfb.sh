...
repo=${DWM_LARGE_SURFACE_REPO:-$repo}
...
if ! grep -Eq '"popupTimeoutMs"[[:space:]]*:[[:space:]]*4000' \
	"$config_home/lyona/notification-settings.json"; then
	printf 'Notification popup timeout was not persisted:\n' >&2
	sed -n '1,20p' "$config_home/lyona/notification-settings.json" >&2
	exit 1
fi
...
[ "$(ipc notifications doNotDisturb)" = true ]
[ "$(ipc notifications popupTimeout)" = 4000 ]
chmod 000 "$config_home/lyona/notification-settings.json"
touch "$config_home/lyona/notification-settings.json"
i=0
while [ "$i" -lt 100 ]; do
	[ "$(ipc notifications policyState)" = unavailable ] && break
	i=$((i + 1))
...
chmod 600 "$config_home/lyona/notification-settings.json"
touch "$config_home/lyona/notification-settings.json"
i=0
while [ "$i" -lt 100 ]; do
	[ "$(ipc notifications policyState)" = available ] && break
...
printf '%s\n' '{"version":1,"doNotDisturb":true,"popupTimeoutMs":6000}' \
	>"$config_home/lyona/notification-settings.json"
i=0
while [ "$i" -lt 100 ]; do
...
printf '%s\n' '{malformed' >"$config_home/lyona/notification-settings.json"
i=0
while [ "$i" -lt 100 ]; do
