...
data_root=$work/data
bin_dir=$work/bin
mkdir -p "$config_home/lyona" "$config_home/alacritty" "$config_home/kitty" \
	"$config_home/gtk-3.0" "$config_home/gtk-4.0" \
	"$data_root/themes/Lyona-nord/gtk-3.0" "$data_root/themes/Lyona-nord/gtk-4.0" \
...
before_hash=$(sha256sum "$config_home/lyona/themes.toml")
before_mode=$(stat -c %a "$config_home/lyona/themes.toml")
output=$(snapshot)
after_hash=$(sha256sum "$config_home/lyona/themes.toml")
after_mode=$(stat -c %a "$config_home/lyona/themes.toml")
...
grep -Fqx $'source\tuser\t'"$config_home/lyona/themes.toml" <<<"$output"
grep -Fqx $'active\tnord\tnord\tselected' <<<"$output"
grep -Fqx $'theme\tnord\tselected\tvalid\ttrue\tLyona-nord\tTheme record is complete' <<<"$output"
...
grep -Fqx $'integration\tgtk\tavailable\tLyona-nord\tRequested GTK theme is installed and applied' <<<"$output"
...
grep -Fqx $'provider\tappearance\tavailable\tread-only\tShared theme inventory and integration state' \
	<<<"$missing_picom"
grep -Fqx $'integration\tgtk\tavailable\tLyona-nord\tRequested GTK theme is installed and applied' \
	<<<"$missing_picom"
...
cp "$config_home/lyona/themes.toml" "$work/replacement-themes.toml"
...
mv "$config_home/lyona/theme-env.sh" "$work/theme-env.sh"
...
grep -Fqx $'error\tqt\tmissing-backend\tNo applied Qt theme backend is recorded' \
	<<<"$no_qt"
mv "$work/theme-env.sh" "$config_home/lyona/theme-env.sh"
...
sed -i 's/^\[active\]$/[active] # selected theme/' "$config_home/lyona/themes.toml"
sed -i 's/^\[theme\.nord\]$/[theme.nord] # default theme/' "$config_home/lyona/themes.toml"
commented_headers=$(snapshot)
grep -Fqx $'provider\tappearance\tavailable\tread-only\tShared theme inventory and integration state' \
	<<<"$commented_headers"
grep -Fqx $'active\tnord\tnord\tselected' <<<"$commented_headers"
grep -Fqx $'theme\tnord\tselected\tvalid\ttrue\tLyona-nord\tTheme record is complete' \
	<<<"$commented_headers"
...
sed -i 's/^\[theme\.dracula\]$/[theme.dracula] trailing/' "$config_home/lyona/themes.toml"
...
rmdir "$data_root/themes/Lyona-nord/gtk-4.0"
partial_gtk_assets=$(snapshot)
grep -Fqx $'integration\tgtk\tpartial\tLyona-nord\tRequested GTK theme is missing GTK 3 or GTK 4 assets' \
	<<<"$partial_gtk_assets"
grep -Fqx $'error\tgtk\tmissing-version\tGTK theme '\''Lyona-nord'\'' does not support both GTK 3 and GTK 4' \
	<<<"$partial_gtk_assets"
...
grep -Fqx $'integration\tgtk\tpartial\tLyona-nord\tRequested GTK theme is installed but not applied' \
	<<<"$stale_gtk_application"
grep -Fqx $'error\tgtk\tstale-theme\tApplied GTK settings do not match '\''Lyona-nord'\''' \
	<<<"$stale_gtk_application"
...
grep -Fqx $'provider\tappearance\tunavailable\tread-only\tShared theme inventory and integration state' \
	<<<"$blocked_user"
grep -Fqx $'source\tuser\t'"$config_home/lyona/themes.toml" <<<"$blocked_user"
...
grep -Fq $'error\tsource\tdangling-user\tIgnoring dangling user theme symlink and trying the managed source: ' \
	<<<"$dangling_user"
rm -f "$config_home/lyona/themes.toml"
...
sed -i "0,/theme = \"nord\"/s//theme = \"$custom_theme\"/" "$config_home/lyona/themes.toml"
sed -i "0,/^\[theme\.nord\]$/s//[theme.$custom_theme]/" "$config_home/lyona/themes.toml"
custom=$(snapshot)
grep -Fqx $'active\t'"$custom_theme"$'\t'"$custom_theme"$'\tselected' <<<"$custom"
grep -Fq $'theme\t'"$custom_theme"$'\tselected\tvalid\ttrue\tLyona-nord' <<<"$custom"
...
cp "$work/managed-themes.toml" "$config_home/lyona/themes.toml"
sed -i '0,/theme = "nord"/s//theme = 123/' "$config_home/lyona/themes.toml"
...
cp "$work/managed-themes.toml" "$config_home/lyona/themes.toml"
sed -i "0,/theme = \"nord\"/s//theme = \"$overlong_theme\"/" "$config_home/lyona/themes.toml"
sed -i "0,/^\[theme\.nord\]$/s//[theme.$overlong_theme]/" "$config_home/lyona/themes.toml"
overlong=$(snapshot)
...
cp "$work/managed-themes.toml" "$config_home/lyona/themes.toml"
sed -i '0,/theme = "nord"/s//theme = "missing"/' "$config_home/lyona/themes.toml"
unknown=$(snapshot)
...
cp "$work/managed-themes.toml" "$config_home/lyona/themes.toml"
sed -i '0,/theme = "nord"/s//theme = "broken"/' "$config_home/lyona/themes.toml"
printf '\n[theme.broken]\ndark_mode = true\n' >>"$config_home/lyona/themes.toml"
incomplete=$(snapshot)
...
grep -Fqx $'theme\tnord\tselected\tinvalid\tbogus value\tLyona-nord\tTheme record is duplicate, malformed, or incomplete' \
	<<<"$invalid_dark"
...
grep -Fqx $'theme\tlegacy\tselected\tvalid\ttrue\tLyona-nord\tTheme record is complete' \
	<<<"$legacy_collision"
...
grep -Fqx $'theme\tnord\trecovery\tvalid\ttrue\tLyona-nord\tTheme record is complete' \
	<<<"$nord_after_cap"
...
grep -Fqx $'theme\tchosen\tselected\tvalid\ttrue\tLyona-nord\tTheme record is complete' \
	<<<"$selected_after_cap"
sed -i '0,/theme = "chosen"/s//theme = chosen/' "$config_home/lyona/themes.toml"
...
mkdir -p "$work/fallback-home/.config/lyona"
cp "$work/managed-themes.toml" "$work/fallback-home/.config/lyona/themes.toml"
relative_xdg=$(HOME="$work/fallback-home" XDG_CONFIG_HOME=relative-config \
	...
	"$helper" snapshot)
grep -Fqx $'source\tuser\t'"$work/fallback-home/.config/lyona/themes.toml" \
	<<<"$relative_xdg"
grep -Fqx $'active\tnord\tnord\tselected' <<<"$relative_xdg"

rm -f "$config_home/lyona/themes.toml" "$work/missing-managed.toml"
set +e
