...
grep -Eq 'cmd="dwm-screenshot clip"' "$repo/config/hotkeys.toml"
...
if find "$work/runtime/lyona" -type f -name 'clipboard.*.png' | grep -q .; then
...
grep -Fqx "notify:-a lyona Screenshot saved $screen_path" "$log"
...
if env DISPLAY=:99 \
	...
	TEST_MAIM_FAIL=1 \
	XDG_RUNTIME_DIR="$work/runtime" \
	"$repo/scripts/dwm-screenshot" clip; then
...
env -u DISPLAY PATH="$work/bin:/usr/bin:/bin" \
	"$repo/scripts/dwm-screenshot" setup

if env -u DISPLAY PATH="$work/bin:/usr/bin:/bin" \
	"$repo/scripts/dwm-screenshot" full 2>"$work/no-display.err"; then
...
if DISPLAY=:99 HOME="$work/home" PATH="$missing_bin" \
	"$repo/scripts/dwm-screenshot" full 2>"$work/no-maim.err"; then
