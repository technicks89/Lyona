...
ShellRoot {
    ...
    function advance() {
        ...
        if (root.stage === 0 && model.snapshotState === "loaded" && !model.busy) {
            ...
            model.actions = availableActions.map(action => action.id === "updates-install-all"
                ? Object.assign({}, action, {status: "unavailable", detail: ""}) : action);
            ...
            root.click("prepareInstall");
            model.refresh();
            // Lyona: discoveryModel/operationModel signals invalidate a stale
            // confirmation eagerly (live watch events only); a plain refresh()
            // instead relies on confirmUpdate()'s own epoch/generation check
            // to reject and then clear it -- evaluate confirmUpdate() first so
            // this still proves a replacement read's confirmation can never
            // dispatch, and is cleared once an attempt is actually made.
            root.check(!model.confirmUpdate() && model.updateConfirmation === null, "Replacement read invalidates confirmation immediately");
            root.stage = 3;
    ...
    // Lyona: SystemSettingsPane requires its own separate, unrelated
    // self-update (lyona-update/lyona-version) model alongside
    // systemManagementModel -- upstream's SystemSettingsPane at this
    // harness's origin (#240) predates that requirement. Nothing in this
    // harness exercises it; it only needs to mount without eagerly failing.
    UpdateModel {
        id: updateModel
    }
    ...
    Window {
        ...
        SystemSettingsPane { anchors.fill: parent; anchors.margins: 12; systemManagementModel: model; updateModel: updateModel; capabilities: [] }
    }
