...
mkdir -p "$work/bin" "$work/config/lyona" "$work/home" "$work/state"
: >"$work/actions.log"
...
write_initial_config() {
	cat >"$work/config/lyona/power.conf" <<'EOF'
...
lock_after=5
EOF
	chmod 600 "$work/config/lyona/power.conf"
...
# Lyona autostarts dwm-lock-watch unconditionally alongside light-locker, and
# configured_lock_running() must recognize it as running evidence when
# lock_managed=1, without ever starting or stopping it itself.
write_initial_config
printf '600\n' >"$work/state/saver_timeout"
printf '5\n' >"$work/state/lock_after"
cat >"$work/bin/dwm-lock-watch" <<'SH'
#!/bin/sh
sleep 60
SH
chmod +x "$work/bin/dwm-lock-watch"
"$work/bin/dwm-lock-watch" &
watch_pid=$!
DWM_POWER_TEST_LOCKER_PID=$watch_pid
export DWM_POWER_TEST_LOCKER_PID
printf '%s\n' "$watch_pid" >"$work/locker-fixture.pid"
DWM_POWER_TEST_REAL_PGREP=1 run_helper power-lock-snapshot >"$work/managed-lock"
awk -F '\t' '$1 == "power-lock" { if ($2 != "available" || $5 != "yes") exit 1; found = 1 } END { if (!found) exit 1 }' "$work/managed-lock"
sed -i 's/^lock_managed=1$/lock_managed=0/' "$work/config/lyona/power.conf"
DWM_POWER_TEST_REAL_PGREP=1 run_helper power-lock-snapshot >"$work/unmanaged-lock"
awk -F '\t' '$1 == "power-lock" { if ($5 != "no") exit 1; found = 1 } END { if (!found) exit 1 }' "$work/unmanaged-lock"
kill "$watch_pid"
wait "$watch_pid" 2>/dev/null || :
rm -f "$work/locker-fixture.pid"
unset DWM_POWER_TEST_LOCKER_PID
write_initial_config
...
write_initial_config
before=$(sha256sum "$work/config/lyona/power.conf")
...
[ "$(sha256sum "$work/config/lyona/power.conf")" = "$before" ]
...
grep -Fqx '# user power preferences' "$work/config/lyona/power.conf"
grep -Fqx 'custom_key=keep' "$work/config/lyona/power.conf"
grep -Fqx 'dpms_enabled=1' "$work/config/lyona/power.conf"
grep -Fqx 'dpms_timeout=900' "$work/config/lyona/power.conf"
[ "$(stat -c %a "$work/config/lyona/power.conf")" = 600 ]
if find "$work/config/lyona" -maxdepth 1 -name '.power.conf.*' -print -quit | grep -q .; then
...
ln -s "$work/power-target.conf" "$work/config/lyona/power.conf"
...
source_packages=$(bash -c '. "$1"; dwm_packages arch desktop' _ "$repo/scripts/dwm-packages.sh")
