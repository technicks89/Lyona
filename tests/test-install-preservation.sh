...
grep -Fq 'Start LightDM now (optional): sudo systemctl start lightdm.service' \
	"$repo/install.sh"
grep -Fq "sudo make install-system \\" "$repo/install.sh"
grep -Fq "make install-user \\" "$repo/install.sh"
if grep -Fq "sudo make install \\" "$repo/install.sh"; then
...
grep -Fq "HOME=\"\${USER_HOME}\" XDG_RUNTIME_DIR=\"/run/user/\$\$target_uid\" \\" \
	"$repo/Makefile"
grep -Fq "\$(MAKE) install-user " "$repo/Makefile"
grep -Fq 'dwm.desktop not found (run '\''./install.sh'\'')' \
	"$repo/scripts/check-deps.sh"
grep -Fq 'Run: make && sudo make install-system && make install-user' \
	"$repo/scripts/check-deps.sh"
if grep -Eq 'sudo make install($|[[:space:]&;])' "$repo/scripts/check-deps.sh"; then
	printf 'Dependency check still recommends a root user-install stage.\n' >&2
	exit 1
fi
if grep -Eq 'sudo chown (-R|--recursive)' "$repo/install.sh"; then
	printf 'Installer uses a broad recursive ownership repair.\n' >&2
	exit 1
fi
MAKE_DATABASE=$(make -qp -C "$repo" 2>/dev/null || true)
...
grep -Fq "runuser -u \"\${OWNER}\" -- env HOME=\"\${USER_HOME}\" \$(MAKE) all" \
	"$repo/Makefile"
grep -Fq 'dwm is not built. Run make before install-system.' "$repo/Makefile"
grep -Fq 'dwm build input is missing:' "$repo/Makefile"
grep -Fq 'dwm is stale. Run make before install-system.' "$repo/Makefile"
INSTALL_USER_RECIPE=$(sed -n '/^install-user:/,/^uninstall:/p' "$repo/Makefile")
...
RESOLVED_OWNER_HOME=$(env -u SUDO_USER -u USER_HOME -u XDG_CONFIG_HOME \
	-u XDG_DATA_HOME USER=root make -s --no-print-directory -C "$repo" \
...
EXPLICIT_OWNER_HOME=$(env -u SUDO_USER -u XDG_CONFIG_HOME -u XDG_DATA_HOME \
	USER=root make -s --no-print-directory -C "$repo" -f "$OWNER_HOME_MAKEFILE" \
...
printf '%s\n' '# existing hotkeys marker' >"$XDG_CONFIG_HOME/lyona/hotkeys.toml"
cp "$TEST_REPO/config/themes.toml" "$XDG_CONFIG_HOME/lyona/themes.toml"
printf '%s\n' '# existing themes marker' >>"$XDG_CONFIG_HOME/lyona/themes.toml"
printf '%s\n' '# existing rules marker' >"$XDG_CONFIG_HOME/lyona/window-rules.toml"
...
snapshot_file "$XDG_CONFIG_HOME/lyona/hotkeys.toml" "$WORK_DIR/hotkeys.before"
snapshot_file "$XDG_CONFIG_HOME/lyona/themes.toml" "$WORK_DIR/themes.before"
snapshot_file "$XDG_CONFIG_HOME/lyona/window-rules.toml" "$WORK_DIR/window-rules.before"
...
assert_preserved hotkeys "$XDG_CONFIG_HOME/lyona/hotkeys.toml" "$WORK_DIR/hotkeys.before"
assert_preserved themes "$XDG_CONFIG_HOME/lyona/themes.toml" "$WORK_DIR/themes.before"
assert_preserved window-rules "$XDG_CONFIG_HOME/lyona/window-rules.toml" "$WORK_DIR/window-rules.before"
