# Arch Linux-Only Desktop Environment Roadmap

## Mission

lyona is expanding from an opinionated dwm build into a cohesive
X11 desktop environment. The product is a complete Arch Linux desktop installed
from a lyona Arch installer image or onto an existing Arch Linux
installation.
...
- Arch Linux is the only supported distribution, implementation target, and
  release-qualification target.
- The archiso-based lyona install medium is the current canonical image
  base.
...
- The Settings experience will be hybrid: common desktop controls belong in a
  ...
  to trusted Arch tools or narrowly scoped privileged helpers.
...
interaction work is folded into the existing Arch product phases while DWM,
X11, Arch providers, helpers, IPC, and session policy remain authoritative.
...
- Arch packaging and dependency requirements with clean unavailable states
  for missing hardware or services.

### Exit Criteria

- The Settings shell opens and navigates without adding idle polling.
- Read-only capability discovery works on Arch Linux and degrades cleanly when an
  Arch service or hardware capability is absent.
...
- PR #168 delivered the shared Power and session-action models, transactional
  ...
  lifecycle, and Arch package/install integration.
...
- User-initiated Arch update status and installation with transparent logs,
  failure reporting, and reboot guidance.
- Date, time, timezone, locale, user-account, printer, and software-source entry
  points through stable platform services or trusted Arch tools.
...
Deliver a repeatable Arch Linux-only desktop installation and upgrade experience.

### Outcomes

- The archiso-based lyona install medium remains the documented minimal base.
- Standard and NVIDIA archiso profiles, package manifests, repository policy, and
  first-boot behavior are qualified against the supported Arch release.
...
- A clean supported Arch image installs, reboots, and reaches a usable dwm
  session with the documented desktop features.
- Archiso profile syntax, package resolution, ISO construction, and first boot are
  recorded for each released image.
- Unsupported or untested Arch hardware paths are stated precisely in
  release notes.

## Future Evaluation

After the Arch phases are stable, evaluate additional accessibility work,
