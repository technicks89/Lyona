...
Scope {
    ...
    /*
     * Candidates grouped by the key each card renders with. The panes used to
     * call .filter() inside the Repeater's model binding, so every card
     * rescanned the whole candidate list whenever anything changed and handed
     * QML a freshly allocated array, forcing it to rebuild each delegate
     * instead of diffing. Grouping once here makes the binding a lookup.
     *
     * emptyGroup keeps the miss path stable too: returning a new [] per
     * evaluation would rebuild the delegates of every empty card.
     */
    readonly property var emptyGroup: []
    readonly property var candidatesByRole: root.groupBy(root.candidates, "role")
    readonly property var mimeCandidatesByMime: root.groupBy(root.mimeCandidates, "mime")
    ...
    function candidatesForRole(role) {
        return root.candidatesByRole[role] || root.emptyGroup;
    }

    function candidatesForMime(mime) {
        return root.mimeCandidatesByMime[mime] || root.emptyGroup;
    }
