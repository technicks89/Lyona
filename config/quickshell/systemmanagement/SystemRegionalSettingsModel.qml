...
// Phase 9's regionalPreview/prepareRegional()/confirmRegional() family,
// removed by this same item), the same split S1-04's SystemDelegateControls
// already established for delegated actions.
Scope {
    id: root
    ...
    // S1-08 (#275): a prepared timezone/NTP request that has already
    // received its outcome but is waiting on timeReconciliation to admit
    // publication is still "owned" from the caller's perspective -- these
    // three plus reconcileTimePreview() let timeReconciliation coordinate
    // with this model without either one polling the other's internals.
