...
Flickable {
    ...
    readonly property string scalingDetail: {
        let detail;
        if (root.settingsModel.displayDpiSource === "saved")
            detail = "Saved for this user and reapplied at every login.";
        else if (root.settingsModel.displayDpiSource === "session")
            detail = "Set for this session only. Apply keeps it across logins.";
        else
            detail = "Using the X server default of 96 DPI.";
        if (root.settingsModel.displayDpiRecommended > 0
                && root.settingsModel.displayDpiRecommended !== root.settingsModel.displayDpi)
            detail += " The primary output measures "
                + root.settingsModel.displayDpiRecommended + " DPI.";
        return detail + " Applications started after the change pick up the new scale;"
            + " restart dwm and the shell to rescale them too.";
    }
    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            Repeater {
                ...
                delegate: Rectangle {
                    ...
                    Layout.preferredHeight: automaticContent.implicitHeight + Theme.dp(16)
                    ...
                    ColumnLayout {
                        ...
                        Item {
                            id: savedMap
                            Layout.fillWidth: true
                            Layout.preferredHeight: automaticCard.profile.saved ? Theme.dp(80) : 0
        ...
        Rectangle {
            Layout.fillWidth: true
            Layout.preferredHeight: automaticConfirmation.implicitHeight + Theme.dp(16)
        ...
        Rectangle {
            Layout.fillWidth: true
            Layout.preferredHeight: Math.max(96, scalingColumn.implicitHeight + 20)
            color: Theme.controlNormalFill
            border.color: Theme.controlNormalBorder
            border.width: 1
            radius: Theme.largeSurfaceCardRadius
                ...
                RowLayout {
                    ...
                    Rectangle {
                        ...
                        TextInput {
                            ...
                            color: Theme.textStrong
                            font.family: Theme.fontFamily
                            validator: IntValidator { bottom: root.settingsModel.minDisplayDpi; top: root.settingsModel.maxDisplayDpi }
                            onAccepted: root.settingsModel.setDisplayDpi(text)
                        }
                        Binding {
                            target: dpiInput
                            property: "text"
                            value: String(root.settingsModel.displayDpi)
                            when: !dpiInput.activeFocus
                    ...
                    Repeater {
                        ...
                        delegate: ShellButton {
                            ...
                            onActivated: root.settingsModel.setDisplayDpi(modelData.value)
                        }
                    }
                    ...
                    ShellButton {
                        label: "Apply"
                        enabled: !root.settingsModel.previewOperationLocked && dpiInput.acceptableInput
                        onActivated: root.settingsModel.setDisplayDpi(dpiInput.text)
                    }
                    ShellButton {
                        label: "Reset"
                        enabled: !root.settingsModel.previewOperationLocked && root.settingsModel.displayDpiPersisted > 0
                        onActivated: root.settingsModel.resetDisplayDpi()
                    }
                }
