 ...
 * (prepareDelegate()/confirmDelegate()/discardDelegate()), this component
 ...
 * (delegated_command() in scripts/dwm-system-management) -- their launch
 * buttons stay disabled and show that detail rather than a fixed target.
 * printers-open keeps its real target (system-config-printer, confirmed in
 * Arch extra). password-open never named a fixed terminal upstream either
 * (it resolves dynamically via dwm-terminal --print-command), so its copy
 * here is unchanged.
 */
ColumnLayout {
    id: root
    ...
    readonly property var tools: [
        {id: "accounts-open", owner: "accounts", label: "Manage accounts", target: "an account-management tool",
            detail: "No account-management tool is packaged for Arch. See docs/src/settings.md."},
        ...
        {id: "printers-open", owner: "printers", label: "Manage printers", target: "Print Settings (system-config-printer)",
            detail: "Configure printers and queues in the trusted Arch tool."},
        {id: "sources-open", owner: "sources", label: "Manage software sources", target: "/etc/pacman.conf",
            detail: "No interactive repository editor is packaged for Arch. Edit /etc/pacman.conf directly; see docs/src/settings.md."}
    ]
    ...
    component Inventory: ListView {
        id: inventory
        Layout.fillWidth: true
        Layout.preferredHeight: Math.min(contentHeight, Theme.dp(180))
        clip: true
