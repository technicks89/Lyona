 ...
 *   of them.
 * - healthAction reads action.status, not upstream's action.availability --
 *   SystemManagementModel.qml's parseSnapshot() names that field "status"
 *   for every action record, native or information (see canNtp's own S1-08
 *   fix for the same field-naming mismatch).
 * - Recovery guidance text no longer names upstream's own distro-specific
 *   package/rescue tooling; it names Arch/Lyona equivalents instead:
 *   `pacman -Qkk` for installed package file verification, `arch-chroot`
 *   from the Lyona installation media for system rescue, and
 *   `lyona-update rollback` for restoring Lyona-managed configuration files
...
ColumnLayout {
    ...
    readonly property var security: [
        ...
        {id: "firewalld", label: "firewalld"}, {id: "ufw", label: "ufw"}, {id: "nftables", label: "nftables"},
        {id: "root-encryption", label: "Root filesystem encryption"}, {id: "screen-lock", label: "Automatic screen lock"}
    ]
    ...
    PlainText {
        text: "Read-only mounted filesystem overview. Partitioning, formatting, encryption changes and filesystem repair belong in a trusted Arch administration tool with a verified backup. Settings does not perform these operations."
        color: Theme.menuMutedText
    }
    ...
    PlainText {
        text: "These indicators describe detected configuration, not a security audit. Firewall service status does not describe firewall rules. Root encryption does not cover every disk. Automatic locking depends on the current X11 session. Review update availability and restart requirements above. Firewall policy and service administration remain in trusted Arch tools (firewalld, ufw, or nftables directly)."
        color: Theme.menuMutedText
    }
    ...
    ShellButton {
        ...
        enabled: root.healthAction !== null && root.healthAction.status === "available"
    ...
    PlainText {
        text: "System Health scans this desktop and offers only its named, confirmed repairs. For package file corruption, `pacman -Qkk` verifies installed files against their recorded checksums. Review diagnostic details before exporting or sharing them; paths and device names may identify your system. For an interrupted operation, reload status and read its recovery result before deciding whether to retry. A denied change leaves readable information available."
        color: Theme.menuMutedText
    }
    SectionLabel { label: "Reset guidance" }
    PlainText {
        text: "For a desktop preference, use the relevant Settings section's reset or revert control and review its scope first. Keep a backup of personal configuration before restoring it. `lyona-update rollback` restores Lyona-managed configuration files to their previous generation. System Health owns its listed desktop repairs; system rescue uses `arch-chroot` from the Lyona installation media. A factory reset, account deletion, disk erase or operating-system reinstall is not offered here. Back up personal files before those separate recovery procedures."
        color: Theme.menuMutedText
    }
}
