 ...
 * SystemManagementModel's regionalPreview state directly; S1-05 moved that
 * state into its own model (the same split S1-04's SystemDelegateControls
 * already established for delegated actions), so this component follows.
 *
 * The delegated-administration section that used to live here (accounts/
 * password/printers/software-sources) moved to SystemDelegateControls.qml
 * in Sync Sprint 1 S1-04.
 */
ColumnLayout {
    id: root
        ...
        delegate: Rectangle {
            ...
            ColumnLayout {
                ...
                ListView {
                    ...
                    visible: catalogCard.choices.length > 0
                    Layout.fillWidth: true
                    Layout.preferredHeight: Math.min(contentHeight, Theme.dp(144), Math.max(0, root.viewportHeight))
