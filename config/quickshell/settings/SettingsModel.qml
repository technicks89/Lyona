...
Scope {
    ...
    property int displayDpiRecommended: 0
    property var inputDevices: []
    property var inputSettings: []
    property var inputUnsupported: []

    /*
     * Grouped by device, for the same reason DefaultAppsModel groups its
     * candidates: the per-device cards used to .filter() these two lists in a
     * property binding, so every card rescanned both whenever either changed.
     */
    readonly property var emptyGroup: []
    readonly property var inputSettingsByDevice: root.groupByDevice(root.inputSettings)
    readonly property var inputUnsupportedByDevice: root.groupByDevice(root.inputUnsupported)

    function groupByDevice(entries) {
        const grouped = {};
        for (const entry of entries) {
            if (grouped[entry.device] === undefined)
                grouped[entry.device] = [];
            grouped[entry.device].push(entry);
        }
        return grouped;
    }

    function inputSettingsFor(device) {
        return root.inputSettingsByDevice[device] || root.emptyGroup;
    }

    function inputUnsupportedFor(device) {
        return root.inputUnsupportedByDevice[device] || root.emptyGroup;
    }
    ...
    readonly property int minDisplayDpi: 72
    readonly property int maxDisplayDpi: 384
    ...
    function parseDisplays(text) {
        ...
        for (const line of text.trim().split("\n")) {
            ...
            } else if (fields[0] === "dpi" && fields.length >= 5) {
                dpi = Number(fields[1]);
                dpiSource = fields[2];
                dpiPersisted = fields[3] === "none" ? 0 : Number(fields[3]);
                dpiRecommended = fields[4] === "none" ? 0 : Number(fields[4]);
            }
        }
        ...
        root.displayDpi = dpi;
        root.displayDpiSource = dpiSource;
        root.displayDpiPersisted = dpiPersisted;
        root.displayDpiRecommended = dpiRecommended;
    ...
    function setDisplayDpi(value) {
        const dpi = Math.round(Number(value));
        if (!isFinite(dpi) || dpi < root.minDisplayDpi || dpi > root.maxDisplayDpi) {
            root.displayMessage = "DPI must be an integer between "
                + root.minDisplayDpi + " and " + root.maxDisplayDpi;
            return;
        }
        root.runDisplay("dpi-set", [String(dpi)]);
    }

    function resetDisplayDpi() {
        root.runDisplay("dpi-reset", []);
    }
