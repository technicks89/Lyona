...
another display or missing display evidence cannot establish readiness.
Startup, shutdown, and rollback use that same display scope so a failed local
change cannot terminate another X session's locker. Lyona additionally
autostarts `dwm-lock-watch`, a reactive watcher for logind's `Lock` signal
that runs independently of X11-idle timeouts; when `power_lock_managed=1` the
`RUNNING` column also treats `dwm-lock-watch` as evidence the configured lock
is active, alongside `light-locker`. Power Settings and Control Center show
Unknown for non-available lock records and clear unverified boolean and
