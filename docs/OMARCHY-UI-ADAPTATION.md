...
lyona borrows the visual grammar of Omarchy's Quickshell menus without
adopting its Wayland or Hyprland runtime. The result must feel related while
remaining an Arch Linux X11 desktop whose window manager, providers, IPC, and
session policy continue to belong to lyona.

This is a design influence, not a shell transplant. Omarchy is MIT licensed;
the implementation here uses independently written lyona QML backed by the
...
New menus must use the existing lyona popup and panel primitives. They
...
existing Arch roadmap. The UI identifiers preserve the approved design-plan
...
existing lyona surface directly. Type to search commands and applications
across the catalog.
