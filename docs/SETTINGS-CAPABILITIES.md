...
| System and diagnostics | `dwm-system-health` structured snapshots and the full-screen health window; `lyona-update`/`lyona-version` versioned protocols for the update surface | Allowlisted user repairs; installed-helper privileged repairs; selected trusted-tool entry points; `lyona-update check/apply/rollback/set-channel` with one confirmed privileged install step | Read-only, user-session, privileged, delegated, unsupported | Authorization denial produces a restricted partial report; high-risk administration stays delegated or unsupported; an unreachable update server reports offline rather than an error, and a declined or failed privileged install step leaves a staged, verified, uninstalled update | `make check-system-health check-quickshell-health-xvfb check-lyona-update check-quickshell-update-model` |
...
| Updates, users, printers, locale/timezone, software sources | No unified provider; some future actions may open trusted Arch tools | Delegated or unsupported | Phase 6 must choose a stable service/tool per operation and keep high-risk administration out of QML. |
