...
| Appearance and accessibility | Versioned `dwm-settings-appearance` theme snapshot and bounded asset inventory; `dwm-settings-wallpaper`; `dwm-settings-font`; five `settings-protocol 1` accessibility capability records from bounded font, D-Bus notification-owner, and XInput probes; `Theme.qml`, `dwm-quickshell-controlcenter`, `theme-apply.sh`, `feh`, and delegated `nwg-look` | User theme, wallpaper, managed-shell font, and text-scale state; toolkit config; external GTK tool | Read-only, user-session, delegated, unsupported | Invalid theme records and unavailable assets are attributed per capability; theme, wallpaper, and managed-shell font controls provide bounded preview and recovery, cursor, icon, GTK, and Qt overrides have their own bounded-preview contract through `dwm-settings-toolkit`; text scaling reflects the same font provider, while contrast, reduced motion, notification, and input-accessibility discovery report current policy gaps until dedicated mutation lands | `make check-appearance check-settings check-quickshell-controlcenter check-quickshell-qml`; nested-X11 appearance lifecycle |
| System and diagnostics | `dwm-system-health` structured snapshots and the full-screen health window; `lyona-update`/`lyona-version` versioned protocols for the update surface; `dwm-system-management` (Arch package updates and regional/account/printer/source management, ported from upstream — see [`UPSTREAM-SYNC.md`](UPSTREAM-SYNC.md#the-system-management-port)) | Allowlisted user repairs; installed-helper privileged repairs; selected trusted-tool entry points; `lyona-update check/apply/rollback/set-channel` with one confirmed privileged install step; `dwm-system-management`'s PackageKit-mediated update install and delegated regional/account/printer/source actions | Read-only, user-session, privileged, delegated | Authorization denial produces a restricted partial report; high-risk administration stays delegated; an unreachable update server reports offline rather than an error, and a declined or failed privileged install step leaves a staged, verified, uninstalled update; `dwm-system-management` provides through `SYNC-P1`'s protocol-minor staging table, never advertising a not-yet-implemented capability as `unsupported` (a status, not a class) | `make check-system-health check-quickshell-health-xvfb check-lyona-update check-quickshell-update-model check-system-management check-quickshell-system-management` |
...
| Cursor, icon, GTK, and Qt overrides | `dwm-settings-toolkit` action protocol version 1.0 over `personalization.conf`, consumed by `theme-apply.sh` | User-session | Each capability is overridden independently and released with a sentinel that hands it back to the active theme, so an override survives a theme change and releasing one leaves the others alone. Rolling back is a re-run of the applier rather than a restore of the files it derives. Preview is bounded and watchdog-backed; mutation is refused in both directions while a theme transaction or toolkit preview is armed; and an apply is confirmed against what the applier reports as live, since a missing GTK theme makes it fall back and still exit 0. |
...
| Notification and accessibility capability discovery | Five `settings-protocol 1` records derived from bounded font, notification-owner, and XInput probes | Read-only | Text scaling reflects the managed font provider's complete versioned status record. High contrast and reduced motion report current policy gaps. Notification and input readiness degrade independently. Dedicated mutation controls remain future work (contrast/motion: Sync Phase 5/6; notification policy: Sync Phase 8). |
...
| Arch package updates, timezone/NTP/locale, accounts, printers, software sources | `dwm-system-management` over PackageKit (`libpk_backend_alpm.so`) and systemd's `timedate1`/`locale1`/`Accounts`; delegated tools for account/printer/source mutation | Read-only, delegated | Ported from upstream (`docs/P6-SYSTEM-MANAGEMENT.md`, `docs/UPSTREAM-SYNC.md`'s system-management port), landing across `SYNC-P1`–`SYNC-P9`. A capability not yet implemented is *omitted* by selecting the highest fully implemented protocol minor — never reported `unsupported`, which is a status a *shipped* provider can report, not a stand-in for "not built yet." |
| Partitions, arbitrary services, firewall policy | Explicitly outside the current helper allowlist | Read-only | Reports an `unsupported` status; delegate to trusted administration tools unless a later specification defines a narrow contract. |

## Arch Provider Matrix
...
duplicate Arch package lists.

| Provider capability | Arch path | Missing capability behavior |
| --- | --- | --- |
| Quickshell Settings frontend | `arch:desktop` | Missing Quickshell makes the Settings UI unavailable. |
| X11 display state | `arch:x11` | Missing RandR tools disable display controls. TearFree and NVIDIA composition remain driver-dependent. |
| NetworkManager | `arch:desktop-optional`; enabled by the Arch package set | Missing service reports unavailable while other sections continue. |
| BlueZ and D-Bus JSON parsing | `bluez`, `systemd`, and `jq` from `arch:desktop` plus the image service/package set | Adapter absence is a runtime unsupported state. |
| PipeWire/WirePlumber and controls | `arch:desktop`; included by the image | Missing session services report unavailable. |
| UPower, Power Profiles, and D-Bus monitoring | `upower`, `power-profiles-daemon`, and `dbus-tools` in `arch:desktop`; included by the Arch package set | Existing installs retain any provider of `ppd-service`; missing battery hardware or profile service disables only the corresponding Power controls. |
| DPMS and auto-lock | X11 tools plus `arch:desktop` | Missing schemas or locker disable only lock controls. |
| Defaults and autostart | `xdg-utils` from `arch:runtime-required`; `inotify-tools` from `arch:desktop` | Missing XDG utilities disable only default-application controls; missing inotify keeps snapshots/actions usable but disables live refresh. |
| Themes and GTK integration | `arch:theme`, `arch:theme-gtk`, and optional profiles | Missing optional theme packages do not disable Settings. |
| Polkit authorization | Arch Linux desktop/image polkit agent and trusted helper | Missing authorization leaves read-only state available. |
| System health | Arch/systemd providers | Missing commands, services, hardware, or authorization emit partial or restricted records. |
...
- Provider failures must be per section. Missing Quickshell, NetworkManager,
  BlueZ, PipeWire, X11, or an Arch Linux-only tool must not damage the core session.
