...
`${XDG_CONFIG_HOME:-$HOME/.config}/lyona/self-heal.path` (one path, no shell
...
`preview-profile`, `keep`, `revert`, `install-profile`, `rollback-system`,
`dpi-set`, and `dpi-reset`) plus the read-only `preview-status` recovery query
and the startup-only `dpi-apply-saved` action,
...
| `unsupported` | This phase or Arch capability has no supported provider contract. |
...
Real Arch evidence must record release, architecture, session type, Settings
...
- Fedora Linux 44, x86_64, X11 reported the `arch` provider family. RandR,
...
Pipeline capability, profiles, persistence, and one `dpi` record carrying the
active Xft DPI, its source (`saved`, `session`, or `default`), the saved value,
and the primary output's measured DPI. Input records describe stable
...
Display scaling is a direct user-session change rather than a timed preview,
because a wrong Xft DPI leaves the session readable and is reversible from the
same pane. `dpi-set` validates an integer between 72 and 384, writes
`dpi.Xresources` through a temporary file and rename, and merges the value into
the running resource database; `dpi-reset` removes the saved value and reloads
the database without `Xft.dpi`, preserving every other resource. Neither action
follows a symlinked resource file. Autostart calls `dpi-apply-saved` before any
application starts so the whole session shares one scale.
...
card, then choose **Left of**, **Right of**, **Above**, or **Below**.
Horizontal placements align top edges; vertical placements align left edges.
Each action moves that monitor and normalizes the active layout origin.
Other monitors retain their relative positions. Resolution and rotation
determine tile size; after changing either, use a placement button again to
align the edges. Discovery appends `mode-size` records (output, mode name,
rate, pixel width, pixel height) so custom RandR names such as `native`
remain positionable. Older discovery responses fall back to conventional
mode names or the active output geometry where available.
...
does not change the live layout. Existing saved coordinate profiles still
load.
...
The optional `autorandr` package (`pacman -S autorandr`) supplies login and
DRM hotplug hooks. Without it, automatic-profile controls explain that
dependency while normal display controls remain usable. Saving Undocked sets
`default -> mobile` and stores only the built-in monitor identity; Docked
stores the connected monitor identity set. Unrecognized hardware uses the
default fallback. Existing XDG autostart disablement is respected; Settings
does not enable system services. Profile replacements preserve hooks and
create backups under `~/.config/lyona/display-profile-backups/` (respecting
`XDG_CONFIG_HOME`). An existing `~/.autorandr` directory takes precedence,
matching autorandr; its backups are stored in `~/lyona/display-profile-backups/`.
...
Named Lyona layouts and privileged **Use at next login** remain separate
...
`/etc/X11/xorg.conf.d/90-lyona-display.conf`. Root-container tests cover
